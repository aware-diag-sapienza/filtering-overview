...
window.lib = (new class {
  ...
  createViews () {
    const _this = this
      ...
      const isLast = (i == _this.dataset.selectorBlocks.length - 1)
  ...
  createSelectorBlock (selectorBlock, width = 250) {
    ...
    const blockBody = block.append('div').attr('class', 'selector-block-body')
  ...
  appendBoxplotSelector (selectorBlockBody, selector) {
    const _this = this
      ...
      .each(function (interval) {
        const w = d3.select(this).style('width').replace('px', '') - _this.constants.outlierPointSize
  ...
  appendActiveDomain (selectorBlockBody, selectorBlock) {
    const _this = this
      ...
      const x = xScale(v[selectorBlock.property])
  ...
  createScatterplot () {
    ...
    var svg = div.append('svg')
      ...
      .each(function (d) {
        const v = d3.entries(d).filter(el => {
...
class Interval {
  constructor (datasetValues, type, property, index, min, max, includeMin, includeMax, active = true, parent = true) {
    this.id = type + '-' + 'interval-' + property.replace(/[^\w]|_/g, '') + '-' + index
      ...
      const x = datasetValues[i][property]
  ...
  updateValues () {
    const _this = this
      ...
      const x = _this.datasetValues[i][_this.property]
...
class Dataset {
  ...
  getInterval (id) {
    const all = this.getAllIntervals()
        return all.filter(function (int) { return int.id == id })[0]
    }

  getInterval2 (id) {
    const all = this.getAllIntervals2()
        return all.filter(function (int) { return int.id == id })[0]
    }
  ...
  onIntervalSelectionChange () {
    const _this = this
        ...
        for (let i = 0; i < _this.values.length; i++) {
      const changed = _this.updateValueSelection(_this.values[i])
  ...
  onIntervalSelectionChange2 () {
    const _this = this
      ...
      const changed = _this.updateValueSelection2(_this.values2[i])
